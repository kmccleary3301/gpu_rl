...
- `compare_use_bonus`: optional positive shaping, currently `0.02`
...
- `near_miss_progress_bonus`: small positive shaping for structured optimize near-misses that use patch plus compare correctly but fail final closeout

Rules:

- Correctness and task success dominate the signal.
- `compare_use_bonus` is intentionally too small to dominate correctness.
...
In the final local v1 ledger, a bounded `near_miss_progress_bonus` exists only as shaping for RL-style optimization traces. It is intentionally too small to let near-misses outrank true successful closes.
