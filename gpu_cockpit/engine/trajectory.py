    ...
    if section == "summary":
        return {
            "run": {
                ...
                for key in ["run_id", "task_id", "status", "trace_enabled", "backend", "vendor", "exit_code", "duration_ms"]
            }
        }
    section_map = {
...
def export_episode_dataset(
    ...
    *,
    ...
    for episode in episodes:
        ...
        readiness_labels.append(str(episode.metadata.get("training_example_kind", "unusable")))
...
def capture_run_episode(root: Path, run_ref: str, *, policy_id: str = "reference_policy_v1", section: str = "full") -> TrajectoryEpisode:
    ...
    reward_components.setdefault("evidence_quality", float(evidence_quality.get("overall_score", 0.0)))
    ...
    for relative_path in episode_refs:
        ...
        if source_run_dir.exists():
            for artifact_ref in episode.artifact_refs:
                if artifact_ref and not (source_run_dir / artifact_ref).exists():
