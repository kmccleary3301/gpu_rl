...
class TopKRouterBenchmarkAdapter(BenchmarkAdapter):
    name = "topk_router"
    case_dir = Path("workloads/benchmarks/topk_router_cases")
    ...
    def load_case(self, root: Path, case_id: str) -> BenchmarkCaseSpec:
        for case in self.list_cases(root):
            if case.case_id == case_id:
                return case
        raise KeyError(f"Unknown top-k router benchmark case: {case_id}")
