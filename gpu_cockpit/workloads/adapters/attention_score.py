...
class AttentionScoreBenchmarkAdapter(BenchmarkAdapter):
    name = "attention_score"
    case_dir = Path("workloads/benchmarks/attention_score_cases")
    ...
    def load_case(self, root: Path, case_id: str) -> BenchmarkCaseSpec:
        for case in self.list_cases(root):
            if case.case_id == case_id:
                return case
        raise KeyError(f"Unknown attention-score benchmark case: {case_id}")
