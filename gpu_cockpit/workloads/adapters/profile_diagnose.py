...
class ProfileDiagnoseBenchmarkAdapter(BenchmarkAdapter):
    name = "profile_diagnose"
    case_dir = Path("workloads/benchmarks/profile_diagnose_cases")
    ...
    def load_case(self, root: Path, case_id: str) -> BenchmarkCaseSpec:
        for case in self.list_cases(root):
            if case.case_id == case_id:
                return case
        raise KeyError(f"Unknown profile-diagnose benchmark case: {case_id}")
