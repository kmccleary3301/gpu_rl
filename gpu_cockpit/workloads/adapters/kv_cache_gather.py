...
class KVCacheGatherBenchmarkAdapter(BenchmarkAdapter):
    name = "kv_cache_gather"
    case_dir = Path("workloads/benchmarks/kv_cache_gather_cases")
    ...
    def load_case(self, root: Path, case_id: str) -> BenchmarkCaseSpec:
        for case in self.list_cases(root):
            if case.case_id == case_id:
                return case
        raise KeyError(f"Unknown kv-cache-gather benchmark case: {case_id}")
