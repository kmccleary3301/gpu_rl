...
class ReductionSumBenchmarkAdapter(BenchmarkAdapter):
    name = "reduction_sum"
    case_dir = Path("workloads/benchmarks/reduction_sum_cases")
    ...
    def load_case(self, root: Path, case_id: str) -> BenchmarkCaseSpec:
        for case in self.list_cases(root):
            if case.case_id == case_id:
                return case
        raise KeyError(f"Unknown reduction-sum benchmark case: {case_id}")
