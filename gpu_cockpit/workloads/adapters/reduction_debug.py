...
class ReductionDebugBenchmarkAdapter(BenchmarkAdapter):
    name = "reduction_debug"
    case_dir = Path("workloads/benchmarks/reduction_debug_cases")
    ...
    def load_case(self, root: Path, case_id: str) -> BenchmarkCaseSpec:
        for case in self.list_cases(root):
            if case.case_id == case_id:
                return case
        raise KeyError(f"Unknown reduction-debug benchmark case: {case_id}")
