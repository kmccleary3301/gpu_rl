...
class RoutingArgmaxBenchmarkAdapter(BenchmarkAdapter):
    name = "routing_argmax"
    case_dir = Path("workloads/benchmarks/routing_argmax_cases")
    ...
    def load_case(self, root: Path, case_id: str) -> BenchmarkCaseSpec:
        for case in self.list_cases(root):
            if case.case_id == case_id:
                return case
        raise KeyError(f"Unknown routing-argmax benchmark case: {case_id}")
