...
def _episode_score(payload: dict[str, Any], row: dict[str, Any]) -> tuple[float, list[str]]:
    ...
    elif str(row.get("quality_bucket")) == "near_miss":
        score += 1.4
    ...
    if int(row.get("branch_depth", 0)) >= 3:
        score += 0.25
        reasons.append("deep_branch_trace")
    if str(row.get("benchmark_track")) == "official":
        score -= 0.25
        reasons.append("heldout_reference_penalty")
...
def _build_example(
    *,
    ...
    prompt = (
        ...
        f"Freeze metadata:\n{json.dumps({'quality_bucket': row['quality_bucket'], 'slice_role': row['slice_role'], 'provenance_kind': row['provenance_kind'], 'branch_depth': row['branch_depth']}, indent=2, sort_keys=True)}\n\n"
        f"Observation packet:\n{json.dumps(observation_packet, indent=2, sort_keys=True)}"
    )
    return SFTExample(
        example_id=f"phase5_freeze_rwr__{Path(str(row['episode_ref'])).stem}__{dup_index}__{turn_index}",
        ...
        metadata={
            ...
            "scoring_policy": "phase5_freeze_rwr_v2",
        },
    )
    ...
    *,
    ...
    for row in rows:
        ...
        if not isinstance(turns, list):
            continue
        score, reasons = _episode_score(payload, row)
        score_table[str(row["episode_ref"])] = {
            "score": score,
            "reasons": reasons,
            "success": bool(payload.get("success")),
    ...
    manifest = SFTDatasetManifest(
        dataset_id=f"phase5_freeze_rwr_{split}_v2",
        created_at=datetime.now(tz=UTC),
        split=split,
        ...
        metadata={
            ...
            "provenance_kind_counts": _count_examples_by(examples, "provenance_kind"),
            "branch_depth_counts": _count_examples_by(examples, "branch_depth"),
            "action_counts": _count_examples_by(examples, "selected_action"),
            "episode_scores": score_table,
        },
    )
...
def main() -> int:
    ...
    parser.add_argument(
        "--train-out-dir",
        type=Path,
        default=ROOT / "datasets" / "phase5_freeze_rwr_train_v2",
    )
    parser.add_argument(
        "--dev-out-dir",
        type=Path,
        default=ROOT / "datasets" / "phase5_freeze_rwr_dev_v2",
    )
    args = parser.parse_args()

    manifest_path = args.freeze_dir / "optimize_trace_manifest.json"
    report_path = args.freeze_dir / "freeze_rwr_dataset_report.json"
    ...
    report = {
        "report_id": f"{args.freeze_dir.name}_freeze_rwr_dataset",
        ...
        "notes": [
            "Branch-aware narrow-RL view built directly from the canonical Phase 5 hard freeze.",
            "Scores combine success, near-miss quality, branch depth, and actual compare/branch/revert/promote evidence.",
            "Official held-out KB-v3 traces remain present for dev analysis but are mildly penalized for train weighting.",
        ],
    }
