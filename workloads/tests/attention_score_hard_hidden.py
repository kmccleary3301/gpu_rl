...
from workloads.reference.triton_attention_score_hard_shared import HIDDEN_K, HIDDEN_Q, reference_attention_scores
...
def _assert_scores_close(actual: object, expected: list[list[float]]) -> None:
    if not isinstance(actual, list) or len(actual) != len(expected):
        _emit_failure(
            ...
            f"Unexpected hidden hard attention scores: {actual}",
...
def main() -> None:
    ...
    accepted = {
        (
            "replace_cpu_reference_path_with_widehead_triton_kernel_candidate",
            "workloads/reference/triton_attention_score_hard_optimize_candidate.py",
        ),
        (
            "supersede_widehead_triton_kernel_candidate_with_ranked_variant",
            "workloads/reference/triton_attention_score_hard_optimize_candidate_v2.py",
        ),
    }
    ...
    if observed not in accepted:
        _emit_failure(
            ...
            f"Unexpected hard attention optimization summary: {observed}",
            section="hidden_tests",
