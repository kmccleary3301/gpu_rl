...
def main() -> None:
    ...
    if hidden.get("indices") != [0, 4, 8, 3, 0, 0]:
        _emit_failure(
            "hidden_routing_indices_mismatch",
            f"Unexpected hidden routing indices: {hidden.get('indices')}",
            section="hidden_tests",
            observed=hidden.get("indices"),
            expected=[0, 4, 8, 3, 0, 0],
        )
    _assert_close_list(list(hidden.get("values", [])), [2.5, 5.0, -1.0, 0.83, 6.0, 7.1])
    ...
    accepted = {
        (
            "replace_cpu_routing_dispatch_path_with_triton_tileaware_candidate",
            "workloads/reference/triton_routing_dispatch_tileaware_harder_optimize_candidate.py",
        ),
        (
            "supersede_triton_tileaware_dispatch_candidate_with_ranked_variant",
            "workloads/reference/triton_routing_dispatch_tileaware_harder_optimize_candidate_v2.py",
        ),
    }
