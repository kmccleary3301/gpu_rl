...
from workloads.reference.triton_attention_score_hard_shared import VISIBLE_K, VISIBLE_Q, reference_attention_scores
...
def _assert_scores_close(actual: object, expected: list[list[float]]) -> None:
    if not isinstance(actual, list) or len(actual) != len(expected):
        _emit_failure(
            ...
            f"Unexpected visible hard attention scores: {actual}",
