...
from workloads.reference.computeeval_reference_runner import _load_problem, hidden_summary
...
def main() -> None:
    ...
    expected = hidden_summary(_load_problem(problem_path))
    actual = payload.get("hidden_result")
    if actual != expected:
        raise SystemExit(f"Unexpected ComputeEval hidden result: {actual} != {expected}")
