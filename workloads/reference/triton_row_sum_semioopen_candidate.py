...
from triton_row_sum_semioopen_kernel import triton_row_sum_semioopen
...
def _run_benchmark(repeats: int) -> None:
    ...
    for _ in range(repeats):
        out = triton_row_sum_semioopen(x)
        torch.cuda.synchronize()
    _ = out


def _payload() -> dict[str, object]:
    visible = triton_row_sum_semioopen(VISIBLE_MATRIX.cuda()).cpu().tolist()
    hidden = triton_row_sum_semioopen(HIDDEN_MATRIX.cuda()).cpu().tolist()
    return {
        "visible_row_sum": [float(value) for value in visible],
        "hidden_row_sum": [float(value) for value in hidden],
        "optimization_summary": {
            "strategy_change": "author_semioopen_triton_row_sum_kernel",
            "candidate_ref": "workloads/reference/triton_row_sum_semioopen_candidate.py",
            "kernel_ref": "workloads/reference/triton_row_sum_semioopen_kernel.py",
            "baseline_ref": "workloads/reference/triton_row_sum_baseline.py"
        },
    }
